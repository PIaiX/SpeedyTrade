...
const UserPage = () => {
  ...
  useEffect(() => {
    getAllGamesWhereUserHasLots(id).then((res) => {
      setGames(res);
  ...
  useEffect(() => {
    activeGame &&
      getLotsByUserAndGame(id, activeGame).then((res) => setLots(res.data));
  }, [activeGame]);
              ...
              <Col xs={12} lg={7}>
                <Row className="mb-4 mb-sm-5">
                  <Col xs={12} sm={5}>
                    {user.isLoaded ? (
                      <img
                        src={getImageURL(user.item?.avatar)}
                        alt=""
                        className="img"
                      />
                    ) : null}
                  ...
                  <Col xs={12} sm={7}>
                    {user.isLoaded ? (
                      <>
                        <h4 className="mb-2">
                          <span className="total-invert">
                            {user.item?.fullName}
                          </span>{" "}
                          -{" "}
                          <span className="achromat-3 fw-4">
                            @{user.item?.nickname}
                          </span>
                        </h4>
                        <div className="achromat-3">
                          {user.item?.isOnline
                            ? "Онлайн"
                            : "Был(а) онлайн " + user.item?.lastSeenForUser}
                        </div>
                      </>
                    ) : null}
                    {user.isLoaded ? (
                    ...
                    <div className="d-flex mt-4 mt-sm-5 text-center">
                      ...
                      <div className="ms-4 ms-xl-5">
                        <div className="num">
                          {user.isLoaded ? user.item?.salesCount : 0}
                        </div>
                        <p className="lh-1 total-invert mt-1">
                          Завершенных сделок
                        </p>
                      </div>
                    </div>
                ...
                <ul className="list-unstyled row row-cols-2 row-cols-md-3 row-cols-lg-2 row-cols-xl-3 g-2 g-sm-3 g-xl-4">
                  {games &&
                    games.slice(0, showAllGames ? 100 : 3).map((game) => (
                      <li key={"game-" + game.id}>
                        <button
                          type="button"
                          className={`game-btn ${
