...
const Parameters = ({
  ...
  useEffect(() => {
    ...
    if (prevChildrenParams) {
      ...
      Object.values(prevChildrenParams).map((children) =>
        children.map((child) => {
    ...
    <>
      ...
      >
        {parameter.isNumeric ? (
          ...
          <>
            {parameter.name}
            <label className="d-flex flex-nowrap gap-2">
              от
              <input
                type="number"
                placeholder="0"
                ...
                onChange={(e) => {
                  setSelectedNumericOptions({
                    ...selectedNumericOptions,
                    [parameter.id]: {
                      ...selectedNumericOptions[parameter.id],
              ...
              e?.target.selectedIndex === 0
                ...
                : parameter.options[e?.target.selectedIndex - 1].childParameter
                  ...
                  setChildrenParam({
                    ...childrenParam,
                    [parameter.id]:
                      parameter.options[e?.target.selectedIndex - 1]
                        .childParameter,
                  });
            }}
          >
      ...
      {childrenParam &&
        childrenParam[parameter.id] &&
        childrenParam[parameter.id].length > 0 && (
          <Parameters
...
const Game = () => {
  ...
  const handleCategorySelection = (category) => {
    // Отправляем выбранную категорию в Redux
  ...
  useEffect(() => {
    ...
    if (catId) {
      ...
      let selectedCategory = game.categories.find((cat) => cat.id == catId);
      if (selectedCategory) {
        handleCategorySelection({ name: selectedCategory.name });
      }
  ...
  useEffect(() => {
    ...
    game.categories[currentCategory.key].parameters?.map((parameter) => {
      if (parameter.displayInLotList)
        setParametersToShow((arr) => [...arr, parameter]);
    });

    game.regions.map(
      (reg) => reg.id == currentRegion && setServers(reg.servers)
    );
  ...
  useEffect(() => {
    if (currentCategory.id && currentRegion) {
      getLotsByCategoryRegionAndParams(
        ...
        JSON.stringify(
          Object.entries(selectedNumericOptions).reduce(
            (obj, item) =>
              Object.assign(obj, { [item[0]]: Object.values(item[1]) }),
            {}
          )
        ),
        onlineOnly,
  ...
  const isCurrency =
    game?.categories &&
    catId &&
    game.categories.filter((c) => c.id === Number(catId))[0]?.isCurrency;
      ...
      <Container>
        <section className="game-page pt-4 pt-sm-5 mb-6 min-vh-100">
          <div className="d-md-flex align-items-center justify-content-between mb-4 mb-sm-2">
            ...
            <BtnAddFav
              favoriteStatus={game?.isFavorite}
              gameId={game.id}
              userId={userId}
            />
          </div>
          ...
          <NavLink
            to="/account/ads/new"
            type="button"
            className="active btn-7 w-xs-100 mb-2 ms-auto me-2 me-lg-4"
            state={{
              selectedOptionGame: {
                label: game?.name,
                value: game?.id,
              ...
              selectedRegion: {
                servers: game?.regions?.find(
                  (element) => element.id == currentRegion
                )?.servers,
                label: game?.regions?.find(
                  (element) => element.id == currentRegion
                )?.name,
                value: Number(currentRegion),
              },
              selectedCategory: {
                label: game?.categories?.find(
                  (element) => element.id == currentCategory?.id
                )?.name,
                value: Number(currentCategory?.id),
              },
              selectedServer: servers
                ? {
                    label: servers?.find(
                      (element) => element.id == currentServer
                    )?.name,
                    value: currentServer,
                  }
                : null,
                ...
                <ul className="lot-line-heading mt-4">
                  ...
                  {/* {parametersToShow.length > 0 &&
                    parametersToShow.map((param) => (
                      <div className="lot-line-cat" key={`param-${param.id}`}>
                        {param.name}
                      </div>

                    ))} */}
                  <div className="lot-line-descr">
                    {isCurrency ? "Доступно" : "Описание"}
                  </div>
                  ...
                  {lots.items?.map((lot) => (
                    <li key={"lot-" + lot.id}>
                      <div className="lot-line">
                        <div
                          onClick={() => nav(`/lot/${lot.id}`)}
                          className="lot-line-server"
                        >
                          {lot.serverName ? lot.serverName : "-"}
                        </div>

                        {/* {parametersToShow.length > 0 &&
                          parametersToShow.map((param) => (
                            <div
                              className="lot-line-cat"
                              key={`param-${param.id}-${lot.id}`}
                              onClick={() => nav(`/lot/${lot.id}`)}
                            >
                              {
                                lot.options.find(
                        ...
                        <div
                          className="lot-line-descr"
                          onClick={() => nav(`/lot/${lot.id}`)}
                        >
                          {isCurrency ? (
                            <div className="clamped">{lot.amount} шт</div>
                          ) : (
