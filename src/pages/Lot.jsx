      ...
      <Container>
        <section className="lot-page pt-4 pt-sm-5 mb-6">
          <Row className="gx-4 gy-5">
            <Col xs={12} lg={7}>
              <h2 className="d-flex flex-column descriptions-lot">
                {lot.item?.description}
                <span className="h3 opacity-75 game-name-lot">
                  {lot.item?.gameInfo?.name}{" "}
                  {lot.item?.serverName && `(${lot.item?.serverName})`}
                  {/* {console.log(lot)} */}
                  {selectedCategory &&
              ...
              <Row className="g-3 g-sm-4">
                ...
                <Col md={9}>
                  <Select
                    ...
                    onChange={(e) =>
                      setPurchaseDto({ ...purchaseDto, paymentType: e.value })
                    }
                  />
                </Col>
                ...
                <Col md={4}>
                  <button
                    ...
                    disabled={
                      !purchaseDto.amount ||
                      purchaseDto.amount < 1 ||
                      purchaseDto.amount > lot.item.amount
                    }
                  >
                    Оплатить{" "}
                    {purchaseDto.amount
                      ? (lot.item.priceCommission * purchaseDto.amount).toFixed(
                          2
                        )
                      : "0"}
                    &nbsp;руб.
                ...
                <Col md={8}>
                  <p className="achromat-3">
                    * Продавец не сможет получить оплату до тех пор, пока вы не
                    подтвердите выполнение всех его обязательств
                  </p>
                </Col>
              </Row>
            ...
            <Col xs={12} lg={5}>
              <div className="message-window">
                {lot.item.user && lot.item.user.id !== userId && (
                  <LotChat lotUser={lot.item.user} />
                )}
              </div>
            </Col>
          </Row>
